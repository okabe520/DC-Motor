	...
	void TIM8_PWM_Init(u32 arr, u32 psc)
{
    ...
    // 使能TIM8和GPIOC、GPIOB时钟
    RCC_APB2PeriphClockCmd(RCC_APB2Periph_TIM8, ENABLE);
    RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOC, ENABLE);
    RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOB, ENABLE);

    // 设置GPIOC6、GPIOB0为TIM8 CH1和CH1N模式，生成PWM输出
    GPIO_PinAFConfig(GPIOC, GPIO_PinSource6, GPIO_AF_TIM8); // GPIOC6复用为TIM8_CH1
    GPIO_PinAFConfig(GPIOB, GPIO_PinSource0, GPIO_AF_TIM8); // GPIOB0复用为TIM8_CH1N

    GPIO_InitStructure.GPIO_Pin = GPIO_Pin_6;            // 配置GPIOC6
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF;         // 复用模式
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_100MHz;   // 速度100MHz
    GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;       // 推挽复用输出
    GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;         // 上拉
    ...
    GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0;           // 配置GPIOB0
    ...
    TIM_OC1Init(TIM8, &TIM_OCInitStructure);               // 初始化TIM8通道1
